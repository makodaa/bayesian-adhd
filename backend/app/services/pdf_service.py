...
DOC_LEFT_MARGIN = 12 * mm
DOC_RIGHT_MARGIN = 12 * mm
DOC_TOP_MARGIN = 12 * mm
...
def build_band_power_block(
    ...
    for i, ((_, _, key), label) in enumerate(zip(bands, labels)):
        ...
        drawing.add(
            String(0, y + 2, label, fontSize=7.5, fontName="Helvetica")
        )
...
def build_disclaimer_box(
    ...
    disclaimer_text = (
        ...
        "to support - not replace - clinical evaluation. All results must be interpreted "
        "by a qualified healthcare professional in the context of the patient's full "
...
class PDFReportService:
    ...
    def _build_report_data(
        ...
        return {
            ...
            "logo_path": None,
            "referral_name": _display(result_data.get("referral_name")),
            "referral_institution": _display(result_data.get("referral_institution")),
    ...
    def _build_summary_disclaimer_columns(self, report_data: dict, width: float) -> Table:
        ...
        disclaimer_block = [
            ...
            build_disclaimer_box(self.styles, width - 6),
        ]
...
def doc_width() -> float:
    return A4[0] - DOC_LEFT_MARGIN - DOC_RIGHT_MARGIN
