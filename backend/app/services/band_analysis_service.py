...
class BandAnalysisService:
    ...
    def compute_band_powers(self, df: pd.DataFrame) -> dict:
        ...
        logger.info(
            f"Band power computation complete. Ratios: theta/beta={result['band_ratios']['theta_beta_ratio']:.4f}"
        )
    ...
    def compute_and_save(self, result_id: int, df: pd.DataFrame) -> dict:
        ...
        logger.info(
            f"Saved {band_power_count} band power entries for result {result_id}"
        )

        # Save ratios
        for ratio, value in powers["band_ratios"].items():
            self.ratios_repo.create_ratio(
                result_id=result_id, ratio_name=ratio, ratio_value=value
            )

        logger.info(
            f"Saved {len(powers['band_ratios'])} ratio entries for result {result_id}"
        )
